...
class roomModel
{
    ...
    public function getAllRooms($page)
    {
        ...
        $query = "SELECT phongtro.*, picture.url, nhatro.DiaChi
                ...
                LEFT JOIN hopdongthue ON hopdongthue.MaPhongTro = phongtro.MaPhongTro
                LEFT JOIN (
                    SELECT MaPhongTro, MAX(url) AS url
                    FROM (
                        SELECT MaPhongTro, url
                        FROM picture
                        ORDER BY RAND()
    ...
    {
        $query = "SELECT DISTINCT phongtro.*, MAX(picture.url) AS url, COUNT(reviews.MaReviews) AS LuotBinhLuan
        FROM phongtro
